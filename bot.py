...
class StatsDatabase:
    ...
    def finish_tournament(
        ...
        self.connection.execute(
            ...
            WHERE tournament_id = ?
            """,
            (json.dumps(winners, ensure_ascii=False), finished_at, tournament_id),
        )
        self.connection.commit()
    ...
    def get_tournament_ticket_rows(self, chat_id: int) -> list[sqlite3.Row]:
        return list(
            self.connection.execute(
                """
                SELECT
    ...
    def finish_referral_contest(
        ...
        self.connection.execute(
            ...
            WHERE contest_id = ?
            """,
            (json.dumps(winners, ensure_ascii=False), finished_at, contest_id),
        )
        self.connection.commit()
...
def format_tournament_ticket_rows(rows: list[sqlite3.Row], limit: int = 5) -> str:
    ...
    for index, row in enumerate(rows[:limit], start=1):
        lines.append(f"{index}. {get_display_name(row)} - {row['total_spins']} билетов")
    return "\n".join(lines)


def get_tournament_total_tickets(rows: list[sqlite3.Row]) -> int:
    return sum(int(row["total_spins"] or 0) for row in rows)
...
def build_tournament_message(
    ...
    ticket_rows = db.get_tournament_ticket_rows(tournament["chat_id"])
    ...
    suffix = (
        ...
        "Условия турнира:\n"
        "1 прокрут = 1 билет.\n"
        "Прокруты до старта турнира тоже считаются.\n"
...
def choose_tournament_winners(
    ...
    candidates = [
        {"row": row, "tickets": int(row["total_spins"] or 0)}
        for row in ticket_rows
        if int(row["total_spins"] or 0) > 0
    ]
    winners = []
...
async def finish_tournament_from_application(
    ...
    db.finish_tournament(tournament["tournament_id"], winners, finished_at)
...
async def finish_referral_contest_from_application(
    ...
    finished_at = datetime_to_storage(utc_now())
    db.finish_referral_contest(contest["contest_id"], winners, finished_at)

    message = build_referral_results_message(contest, winners, len(participant_rows))
    owner_user_id = int(contest["owner_user_id"])
    await application.bot.send_message(chat_id=owner_user_id, text=message)
...
def tournament_usage() -> str:
    return (
        ...
        "Где 7 - дней турнира, 3 - призовых места.\n\n"
        "Другие команды:\n"
...
def parse_tournament_start_command(message_text: str) -> tuple[int, int, int, list[str]]:
...
async def start_tournament(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        chat_id, days, prize_places, prizes = parse_tournament_start_command(
...
def parse_referral_finish_token(token: str) -> tuple[int | None, int | None]:
    normalized = token.strip().lower()
    if normalized.startswith("days"):
        return int(normalized.removeprefix("days")), None
    if normalized.startswith("day"):
        return int(normalized.removeprefix("day")), None
    if normalized.endswith("d"):
        return int(normalized[:-1]), None
    ...
    raise ValueError("bad finish token")
...
def parse_refcontest_start_command(
    ...
    days, max_participants = parse_referral_finish_token(args[1])
    prize_places = int(args[2])
    ...
    return days, max_participants, prize_places, prizes
...
async def start_referral_contest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        days, max_participants, prize_places, prizes = parse_refcontest_start_command(
            update.message.text or ""
        )
    except ValueError:
    ...
    announcement_text, announcement_entities = announcement
    started_at = utc_now()
    ends_at = datetime_to_storage(started_at + timedelta(days=days)) if days else None
