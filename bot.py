...
def get_mines_button_count(db: StatsDatabase) -> int:
    ...
    try:
        return max(MINES_MIN_BUTTONS, int(value))
    except (TypeError, ValueError):
...
def get_mines_count(db: StatsDatabase) -> int:
    ...
    try:
        return max(MINES_MIN_COUNT, min(int(value), button_count - 1))
    except (TypeError, ValueError):
        return max(MINES_MIN_COUNT, min(DEFAULT_MINES_COUNT, button_count - 1))
...
def validate_mines_variant(button_count: int, mine_count: int) -> None:
    if button_count < MINES_MIN_BUTTONS:
        raise ValueError(f"В минках должно быть минимум {MINES_MIN_BUTTONS} кнопки.")
    if mine_count < MINES_MIN_COUNT:
        raise ValueError(f"Мин должно быть минимум {MINES_MIN_COUNT}.")
...
def parse_mines_variation_bulk(
    ...
    if not parsed_variations:
        raise ValueError(
            "Не нашел вариации. Пример строки: 12 4 15,20,30,45,60,80,110,150"
        )
...
def save_mines_settings(
    ...
    if mine_count is not None:
        ...
        if mine_count < MINES_MIN_COUNT:
            raise ValueError(f"Мин должно быть минимум {MINES_MIN_COUNT}.")
        if mine_count >= current_button_count:
            raise ValueError("Мин должно быть меньше, чем кнопок.")
...
def mines_variation_keyboard(
    ...
    buttons = [
        InlineKeyboardButton(
            ...
            callback_data=f"minecfg:{choice_id}:{button_count}:{mine_count}",
        )
        for button_count, mine_count in variations
    ]
...
async def send_mines_variation_choice(
    ...
    sent_message = await trigger_message.reply_text(
        "Выбери поле для минок:",
        reply_markup=mines_variation_keyboard(choice_id, variations),
    )
    db.set_jackpot_game_choice_message_id(choice_id, sent_message.message_id)
...
async def handle_jackpot_game_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "mines":
        ...
        try:
            await query.message.edit_text(
                "Выбери поле для минок:",
                reply_markup=mines_variation_keyboard(
                    choice_id,
                    get_mines_variations(db),
                ),
            )
        except TelegramError:
...
async def handle_mines_variation_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await query.answer("Некорректная вариация минок.", show_alert=True)
        return
    ...
    if (button_count, mine_count) not in set(get_mines_variations(db)):
        await query.answer("Эта вариация сейчас не активна.", show_alert=True)
        return
    ...
    stats = db.get_user_stats(chat_id, query.from_user.id)
    await query.answer(f"{button_count} клеток / {mine_count} мин.", show_alert=True)
    try:
        await query.message.edit_text(
            f"Выбраны минки: {button_count} клеток / {mine_count} мин."
        )
    except TelegramError:
        pass
    await send_mines_challenge_from_message(
...
def game_settings_text(db: StatsDatabase) -> str:
    ...
    return (
        ...
        "/game minescount 2, /game minesstart 15, /game minesstep 5\n"
        "Лестница минок: /game minessteps 12 4 15,20,30,45,60,80,110,150\n"
        ...
        if action in {"classic", "old", "классика", "обычный"} and len(context.args) == 1:
            ...
            try:
                if subaction in {"buttons", "button", "boxes", "box", "кнопки"}:
                    ...
                    if len(numbers) < 2:
                        raise ValueError("Формат: /game mines variant 12 4")
                    ...
                    await update.message.reply_text(
                        "Формат: /game mines buttons 9, /game mines count 2, "
                        "/game mines start 15, /game mines step 5, "
                        "/game mines variant 12 4, /game mines steps 12 4 15,20,30"
                    )
                    return
            except ValueError as error:
                await update.message.reply_text(str(error))
            ...
            if not payload:
                await update.message.reply_text(
                    ...
                    "/game minesbulk\n"
                    "8 2 10,18,30,50,80,120\n"
                    "12 4 15,20,30,45,60,80,110,150"
                )
                return
            try:
                saved_count = 0
            ...
            await update.message.reply_text(
                ...
                "/game minesstart 15\n"
                "/game minesstep 5\n"
                "/game minesvariant 12 4\n"
                "/game minessteps 12 4 15,20,30,45,60,80,110,150\n"
                "/game minesbulk\n"
                "8 2 10,18,30,50,80,120\n"
                "12 4 15,20,30,45,60,80,110,150\n"
                "/game commission 15"
...
def owner_panel_commands_text() -> str:
    return (
        ...
        "/game minescount 2, /game minesvariant 12 4, "
        "/game minessteps 12 4 15,20,30,45,60,80,110,150, /game minesbulk, "
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(handle_mines_variation_choice, pattern="^minecfg:"))
