...
class StatsDatabase:
    ...
    def get_jackpot_button_count(self, user_id: int) -> int:
        ...
        return max(JACKPOT_BUTTONS_MIN, min(JACKPOT_BUTTONS_START, int(row["button_count"] or 0)))

    def set_jackpot_button_count(self, user_id: int, button_count: int) -> None:
        clamped_count = max(JACKPOT_BUTTONS_MIN, min(JACKPOT_BUTTONS_START, button_count))
        self.connection.execute(
            """
...
async def send_jackpot_button_challenge(
    ...
    next_count = max(JACKPOT_BUTTONS_MIN, button_count - JACKPOT_BUTTONS_MISS_DECREASE)
    await update.message.reply_text(
        f"{get_user_display_name(update.effective_user)} выбил 777!\n\n"
        f"Выбери одну из {button_count} коробок.\n"
        "В одной коробке NFT, в остальных подарки на 15⭐ или 25⭐.\n"
        f"Если NFT не выпадет, следующий 777 будет с {next_count} кнопками.",
...
async def handle_jackpot_button_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_nft:
        db.set_jackpot_button_count(query.from_user.id, JACKPOT_BUTTONS_START)
        gift = {
            "title": round_row["nft_title"] or "NFT",
            "url": round_row["nft_url"] or "",
        }
        ...
        await context.bot.send_message(
            ...
            text=(
                f"{player} открыл коробку #{selected_position} и забрал NFT.\n\n"
                f"{gift_text}\n\n"
                f"Следующий 777 снова начнется с {JACKPOT_BUTTONS_START} кнопок."
            ),
        )
    ...
    new_count = max(
        JACKPOT_BUTTONS_MIN,
        int(round_row["button_count"]) - JACKPOT_BUTTONS_MISS_DECREASE,
    )
    ...
    await context.bot.send_message(
        chat_id=round_row["chat_id"],
        text=(
            f"{player} открыл коробку #{selected_position}.\n\n"
            f"Выпал подарок на {small_prize}⭐.\n"
            f"NFT не найден. Следующий 777: {new_count} кнопок."
        ),
    )
    await notify_owners(
        update,
        ...
        config,
        (
            "777-кнопки: промах по NFT\n\n"
            f"Игрок: {player}\n"
            f"Приз: {small_prize}⭐\n"
            f"Следующий 777: {new_count} кнопок"
        ),
    )
...
def build_personal_stats_message(
    ...
    if not template:
        ...
        return build_default_personal_stats_message(user, stats, balance, rank_text, rank_points), []
...
def build_milestone_message(
    ...
    if not template:
        ...
        return build_default_milestone_message(user, stats, balance, rank_text, rank_points), []
...
def build_help_message(
    ...
    if not template:
        return (
            ...
            "/fullstat - полная статистика всех игроков в личке\n"
...
async def set_help_template(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not extracted:
        await update.message.reply_text(
            ...
            "Placeholders: username, balance, tem_balance, total_spins, rank, rank_points"
        )
        return
...
async def manage_game_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args:
        ...
        if action in {"mode", "режим"} and len(context.args) >= 2:
            ...
            try:
                ...
                await update.message.reply_text(str(error))
                return
            db.set_bot_setting(SPIN_PRICE_STARS_SETTING, max(1, price))
        else:
            await update.message.reply_text(
                "Формат:\n"
                "/game\n"
...
async def react_to_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if result == "jackpot":
        if get_game_mode(db) == GAME_MODE_JACKPOT_BUTTONS:
            await send_jackpot_button_challenge(update, context, config, db)
        else:
