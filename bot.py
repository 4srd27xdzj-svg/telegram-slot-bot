    ...
    def set_rank_cards(self, rank_cards: list[dict]) -> None:
        self.connection.execute(
            ...
            ON CONFLICT(setting_key) DO UPDATE SET
                ...
                updated_at = CURRENT_TIMESTAMP
            """,
            (json.dumps(rank_cards, ensure_ascii=False),),
        )
        self.connection.commit()
...
def should_send_chance_hint() -> bool:
    return random.random() < 0.20


def choose_jackpot_chance_percent() -> str:
    if random.random() < 0.88:
        return random.choice(["1.4%", "1.5%", "1.6%", "1.7%", "1.8%", "2.0%", "2.2%"])

    return random.choice(["7%", "9%", "12%", "17%", "25%"])
...
def build_chance_hint_message(db: StatsDatabase, user: User) -> tuple[str, list[dict]]:
    chance_percent = choose_jackpot_chance_percent()
