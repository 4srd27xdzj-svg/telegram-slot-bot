...
def get_rank_level(total_spins: int) -> int:
    return total_spins // 100


def get_rank_name(total_spins: int) -> str:
    rank_level = get_rank_level(total_spins)
    if rank_level < len(DEFAULT_RANK_NAMES):
        return DEFAULT_RANK_NAMES[rank_level]

    return f"легенда {rank_level}"


def get_spins_to_next_rank(total_spins: int) -> int:
    remainder = total_spins % 100
    return 100 if remainder == 0 else 100 - remainder


def rank_values(total_spins: int) -> dict[str, str]:
    rank_level = get_rank_level(total_spins)
    return {
        "rank": get_rank_name(total_spins),
        "rank_level": str(rank_level),
        "next_rank_spins": str((rank_level + 1) * 100),
        "spins_to_next_rank": str(get_spins_to_next_rank(total_spins)),
    }
...
def format_top_spin_rows(rows: list[sqlite3.Row]) -> str:
    if not rows:
    ...
    for index, row in enumerate(rows, start=1):
        lines.append(
            ...
            f"ранг: {get_rank_name(row['total_spins'])}, "
            f"777: {row['jackpots']}, "
...
def build_default_chat_stats_message(totals: sqlite3.Row, top_rows: list[sqlite3.Row]) -> str:
    return (
        "Статистика слотов\n\n"
        "Общая статистика:\n"
        ...
        "Топ 5 по спинам:\n"
        f"{format_top_spin_rows(top_rows)}"
    )
...
def build_chat_stats_message(
    ...
    if not template:
        return build_default_chat_stats_message(totals, top_rows), []
    ...
    values = {
        ...
        "top5": format_top_spin_rows(top_rows),
    }
    return apply_template_values(text, entities_data, values)


def build_default_personal_stats_message(user: User, stats: sqlite3.Row | dict[str, int]) -> str:
    return (
        f"Личная статистика {get_user_display_name(user)}\n\n"
        f"Всего спинов: {stats_value(stats, 'total_spins')}\n"
        f"Ранг: {get_rank_name(stats_value(stats, 'total_spins'))}\n"
...
def build_personal_stats_message(
    ...
    if not template:
        return build_default_personal_stats_message(user, stats), []
    ...
    values = {
        ...
        **rank_values(stats_value(stats, "total_spins")),
    }
    return apply_template_values(text, entities_data, values)


def build_default_milestone_message(user: User, stats: sqlite3.Row | dict[str, int]) -> str:
    total_spins = stats_value(stats, "total_spins")
    return (
        f"{get_user_display_name(user)} достиг {total_spins} спинов.\n\n"
        f"Ранг: {get_rank_name(total_spins)}\n"
        f"777: {stats_value(stats, 'jackpots')}\n"
        f"77X: {stats_value(stats, 'two_sevens')}\n"
        f"Три в ряд: {three_of_kind_total(stats)}"
    )
...
def build_milestone_message(
    ...
    if not template:
        return build_default_milestone_message(user, stats), []
...
async def set_message_template(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "rank - текущий ранг\n"
        "rank_level - номер ранга\n"
        "next_rank_spins - спины для следующего ранга\n"
        "spins_to_next_rank - сколько спинов осталось до следующего ранга"
    )
...
async def show_message_templates(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not templates and not user_templates:
