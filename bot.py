...
class StatsDatabase:
    ...
    def open_payout_dispute(self, payout_id: int, user_id: int, dispute_text: str) -> bool:
        cursor = self.connection.execute(
            ...
            SET status = ?,
                dispute_text = ?,
                completed_at = NULL
            WHERE payout_id = ? AND user_id = ? AND status != ?
            """,
            (
                PAYOUT_STATUS_DISPUTED,
                dispute_text,
    ...
    def create_reward_task(
        ...
        self.connection.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (title, scope, metric, goal, prize_type, prize_value, chat_id, created_by),
        )
        self.connection.commit()
        return int(self.connection.execute("SELECT last_insert_rowid()").fetchone()[0])
    ...
    def ensure_reward_task_participant(
        ...
        self.connection.execute(
            ...
            """,
            (task_id, user_id, baseline_value),
        )
        self.connection.commit()
    ...
    def get_luck_rows(self, chat_ids: set[int], metric: str, reverse: bool) -> sqlite3.Row | None:
        if not chat_ids:
            return None
        ...
        return self.connection.execute(
            ...
            SELECT
                r.user_id,
                COUNT(*) AS attempts,
                SUM(CASE WHEN r.selected_position = r.nft_position THEN 1 ELSE 0 END) AS wins,
                u.username,
                u.first_name,
                u.last_name,
                (SUM(CASE WHEN r.selected_position = r.nft_position THEN 1 ELSE 0 END) * 1.0 / COUNT(*)) AS ratio
            ...
            WHERE r.chat_id IN ({placeholders})
              ...
              AND e.user_id IS NULL
            GROUP BY r.user_id, u.username, u.first_name, u.last_name
            HAVING attempts > 0
            ORDER BY ratio {order_direction}, attempts DESC
            LIMIT 1
            """,
...
async def build_jackpot_button_challenge_message(
    ...
    return (
        ...
        "В одной коробке NFT, в остальных подарки на 15⭐ или 25⭐.\n"
...
async def handle_jackpot_button_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    is_nft = selected_position == int(round_row["nft_position"])
    small_prize = None if is_nft else random.choice(JACKPOT_BUTTON_SMALL_PRIZES)
...
async def complete_payout_workflow(
    ...
    if private_chat_id:
        try:
            await context.bot.send_message(
                chat_id=private_chat_id,
                text=f"Выплата завершена: {payout['prize_text']}",
            )
        except TelegramError as error:
...
def task_progress_line(db: StatsDatabase, task: sqlite3.Row, user_id: int) -> str:
    ...
    current = db.get_user_metric_value(
        int(task["chat_id"] or 0),
        user_id,
        task["metric"],
    ) if task["chat_id"] else 0.0
    progress = max(0.0, current - float(participant["baseline_value"] or 0))
    return f"{format_rank_points(progress)}/{format_rank_points(float(task['goal']))}"
...
async def process_reward_tasks(
    ...
    for task in db.get_active_reward_tasks_for_user(update.effective_chat.id, update.effective_user.id):
        ...
        payout_id = await create_task_payout(
            context,
            config,
            db,
            task,
        ...
        db.complete_reward_task_participant(
            task["task_id"],
            update.effective_user.id,
            payout_id,
        )
        if update.message:
            await update.message.reply_text(
                f"{get_user_display_name(update.effective_user)} выполнил задание #{task['task_id']}.\n"
...
def task_usage() -> str:
    return (
        "Задания owner:\n\n"
        "/task create all spins 500 nft Название\n"
        "/task create optin 777 2 stars25 Название\n"
        "/task create users @user1 @user2 spins 500 nft Название\n"
        "/task list\n"
        "/task stop ID\n\n"
...
def parse_task_create_args(db: StatsDatabase, args: list[str]) -> tuple[str, str, float, str, str, str, list[int]]:
    if len(args) < 5:
    ...
    title = " ".join(args[metric_index + 3:]).strip()
    if not title:
        title = f"{task_metric_label(metric)} x{format_rank_points(goal)}"

    return scope, metric, goal, prize_type, prize_value, title, allowed_user_ids
...
async def manage_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "create":
        try:
            scope, metric, goal, prize_type, prize_value, title, allowed_user_ids = parse_task_create_args(db, context.args)
        ...
        await update.message.reply_text(
            ...
            f"{title}\n"
            f"Тип: {scope}, цель: {format_rank_points(goal)} {task_metric_label(metric)}, приз: {prize_type} {prize_value}"
        )
        return
...
async def show_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    tasks = db.get_visible_reward_tasks_for_user(update.effective_user.id)
    if not tasks:
        await update.message.reply_text("Активных заданий сейчас нет.")
        return
    ...
    for task in tasks:
        participant = db.get_reward_task_participant(task["task_id"], update.effective_user.id)
        status = participant["status"] if participant else task_progress_line(db, task, update.effective_user.id)
        lines.append(
        ...
        if task["scope"] == "optin" and participant is None:
            buttons.append(
                [InlineKeyboardButton(f"Взять #{task['task_id']}", callback_data=f"taskjoin:{task['task_id']}")]
            )
...
async def handle_task_join(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    baseline = 0.0
    if query.message:
        chat_id = int(task["chat_id"] or query.message.chat_id)
        baseline = db.get_user_metric_value(chat_id, query.from_user.id, task["metric"])
    db.ensure_reward_task_participant(task_id, query.from_user.id, baseline)
...
async def show_luck_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"Удача: {scope}\n\n"
        f"Самый везучий по 777:\n{format_luck_row(db.get_luck_rows(chat_ids, '777', True))}\n\n"
        f"Самый невезучий по 777:\n{format_luck_row(db.get_luck_rows(chat_ids, '777', False))}\n\n"
        f"Самый везучий по NFT-кнопкам:\n{format_luck_row(db.get_luck_rows(chat_ids, 'nfts', True))}\n\n"
        f"Самый невезучий по NFT-кнопкам:\n{format_luck_row(db.get_luck_rows(chat_ids, 'nfts', False))}"
    )
...
async def withdraw_stars(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"Остаток баланса: {db.get_stars_balance(update.effective_user.id)}⭐."
    )
...
def owner_panel_tasks_overview(db: StatsDatabase) -> str:
    ...
    lines = [
        ...
        "",
        "Создание:",
        "/task add all spins 500 nft Название",
        "/task add optin 777 3 stars25 Название",
    ...
    for task in tasks[:8]:
        lines.append(
            ...
            f"{task['metric']} {task['goal']} -> {task['prize_type']}"
        )
    return "\n".join(lines)
...
def owner_panel_commands_text() -> str:
    return (
        ...
        "Ранги/шансы/рубежи:\n"
        "/setranks, /setuserrank, /setchance, /milestones\n\n"
        "Статистика:\n"
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(handle_task_join, pattern="^taskjoin:"))
