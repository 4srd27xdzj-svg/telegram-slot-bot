...
def build_full_stats_text(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        rank = get_rank_name(total, rank_names)
        lines.append(
            f"{index}. {get_display_name(row)} - "
            f"спины {total}, "
...
async def hide_user_from_rating(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_owner(config, update.effective_user.id):
        await update.message.reply_text(
            "Эта команда работает только для owner. Проверьте /ownercheck в личке с ботом."
        )
        return
...
async def manage_gift_block(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    config: BotConfig = context.application.bot_data["config"]
    if update.effective_chat.type != "private":
        await update.message.reply_text("Блокировать gifts можно только в личке с ботом.")
        return

    if not is_owner(config, update.effective_user.id):
        await update.message.reply_text("Эта команда работает только для owner.")
        return
...
async def manage_tournament(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type != "private":
        await update.message.reply_text("Команды турнира запускаются только в личке с ботом.")
        return
