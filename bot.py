...
async def hide_user_from_rating(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target_user_id, user_label = resolve_target_user_id_from_update(update, context)
    if target_user_id is None:
        await update.message.reply_text(
            "Ответьте /hiderating на сообщение пользователя или напишите /hiderating USER_ID."
        )
        return

    db: StatsDatabase = context.application.bot_data["db"]
    db.exclude_user_from_rating(target_user_id)
...
async def show_user_in_rating(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if target_user_id is None:
        await update.message.reply_text(
            "Ответьте /showrating на сообщение пользователя или напишите /showrating USER_ID."
        )
        return

    db: StatsDatabase = context.application.bot_data["db"]
    db.include_user_in_rating(target_user_id)
