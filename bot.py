...
JACKPOT_BUTTON_NFT_CHANCE_STEP_SETTING = "jackpot_button_nft_chance_step"
...
MINES_INCREMENT_STARS_SETTING = "mines_increment_stars"
MINES_PRIZE_STEPS_SETTING = "mines_prize_steps"
MINES_VARIATIONS_SETTING = "mines_variations"
MINES_PRIZE_STEPS_PREFIX = "mines_prize_steps:"
MINES_FIELD_OPTIONS = (4, 9, 16)
MINES_MIN_BUTTONS = 4
MINES_MIN_COUNT = 2
DEFAULT_MINES_BUTTONS = 9
DEFAULT_MINES_COUNT = 2
DEFAULT_MINES_START_STARS = 15
DEFAULT_MINES_INCREMENT_STARS = 5
SERVICE_COMMISSION_PERCENT_SETTING = "service_commission_percent"
...
AUTO_BOLD_PLACEHOLDERS = {
    ...
    "mine_count",
    "mine_start_stars",
    "mine_increment_stars",
    "mine_prize_steps",
    "current_prize",
    "opened_count",
    "safe_count",
    "remaining_safe_count",
    "spent_total",
    "service_commission",
    "net_income",
    "prizes_won",
...
TEMPLATE_KEY_ALIASES = {
    ...
    "mines": "mines_start",
    "минки": "mines_start",
    "minesstart": "mines_start",
    "mines_start": "mines_start",
    "minessafe": "mines_safe",
    "mines_safe": "mines_safe",
    "minescash": "mines_cashout",
    "minescashout": "mines_cashout",
    "mines_cashout": "mines_cashout",
    "mineslost": "mines_lost",
    "mines_lost": "mines_lost",
    "minesboom": "mines_lost",
    "77x": "two_sevens",
    "77х": "two_sevens",
...
TEMPLATE_LABELS = {
    ...
    "mines_start": "минки старт",
    "mines_safe": "минки безопасная кнопка",
    "mines_cashout": "минки забрал Stars",
    "mines_lost": "минки мина",
    "two_sevens": "77X",
...
class StatsDatabase:
    ...
    def init_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS jackpot_game_choices (
            ...
            CREATE TABLE IF NOT EXISTS economy_events (
                ...
                source_type TEXT,
                source_id INTEGER,
                amount_stars REAL NOT NULL DEFAULT 0,
                commission_stars REAL NOT NULL DEFAULT 0,
                net_amount_stars REAL NOT NULL DEFAULT 0,
                created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
            );
            ...
            CREATE TABLE IF NOT EXISTS mine_rounds (
                ...
                mine_positions_json TEXT NOT NULL DEFAULT '[]',
                opened_positions_json TEXT NOT NULL DEFAULT '[]',
                prize_steps_json TEXT NOT NULL DEFAULT '[]',
                current_prize_stars INTEGER NOT NULL DEFAULT 0,
                start_prize_stars INTEGER NOT NULL,
                increment_stars INTEGER NOT NULL,
                message_id INTEGER,
                status TEXT NOT NULL DEFAULT 'open',
                created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                resolved_at TEXT
    ...
    def ensure_schema_migrations(self) -> None:
        ...
        mine_round_columns = {
            row["name"]
            for row in self.connection.execute("PRAGMA table_info(mine_rounds)").fetchall()
        }
        if "prize_steps_json" not in mine_round_columns:
            self.connection.execute(
                "ALTER TABLE mine_rounds ADD COLUMN prize_steps_json TEXT NOT NULL DEFAULT '[]'"
            )
            self.connection.commit()
    ...
    def record_economy_event(
        self,
        chat_id: int,
        user_id: int,
        event_type: str,
        amount_stars: float,
        commission_stars: float = 0,
        net_amount_stars: float = 0,
        ...
        self.connection.execute(
            ...
            (
                ...
                source_id,
                float(amount_stars),
                float(commission_stars),
                float(net_amount_stars),
            ),
        )
        self.connection.commit()

    def record_spin_economy(
        self,
        chat_id: int,
        user_id: int,
        spin_price_stars: int,
        commission_percent: float,
        result: str,
    ) -> None:
        gross = float(spin_price_stars)
        commission = gross * max(0.0, commission_percent) / 100.0
        self.record_economy_event(
            chat_id,
            user_id,
            "spin_spend",
            gross,
    ...
    def get_economy_totals(self, chat_ids: set[int] | None = None) -> sqlite3.Row:
        params: list[int] = []
        chat_filter = ""
        if chat_ids:
            placeholders = ",".join("?" for _ in chat_ids)
            chat_filter = f"WHERE chat_id IN ({placeholders})"
            params.extend(sorted(chat_ids))

        row = self.connection.execute(
            f"""
            SELECT
                COALESCE(SUM(CASE WHEN event_type = 'spin_spend' THEN amount_stars ELSE 0 END), 0) AS spent_total,
                COALESCE(SUM(CASE WHEN event_type = 'spin_spend' THEN commission_stars ELSE 0 END), 0) AS service_commission,
                COALESCE(SUM(CASE WHEN event_type = 'spin_spend' THEN net_amount_stars ELSE 0 END), 0) AS net_income,
                COALESCE(SUM(CASE WHEN event_type = 'stars_prize' THEN amount_stars ELSE 0 END), 0) AS prizes_won,
                COUNT(CASE WHEN event_type = 'spin_spend' THEN 1 END) AS paid_spins,
                COUNT(CASE WHEN event_type = 'stars_prize' THEN 1 END) AS prize_events
            FROM economy_events
            {chat_filter}
            """,
            tuple(params),
        ...
        if row is None:
            raise RuntimeError("Economy query returned no totals.")
        return row
    ...
    def create_jackpot_game_choice(self, chat_id: int, user_id: int) -> int:
        self.connection.execute(
            """
            INSERT INTO jackpot_game_choices (chat_id, user_id)
            VALUES (?, ?)
            """,
            (chat_id, user_id),
        )
    ...
    def set_jackpot_game_choice_message_id(self, choice_id: int, message_id: int) -> None:
        self.connection.execute(
            """
            UPDATE jackpot_game_choices
            SET message_id = ?
            WHERE choice_id = ?
            """,
    ...
    def get_jackpot_game_choice(self, choice_id: int) -> sqlite3.Row | None:
        return self.connection.execute(
            """
            SELECT *
            FROM jackpot_game_choices
            WHERE choice_id = ?
            """,
            (choice_id,),
        ).fetchone()

    def expire_open_jackpot_game_choices(self, chat_id: int, user_id: int) -> None:
        self.connection.execute(
            """
            UPDATE jackpot_game_choices
            SET status = 'expired',
    ...
    def resolve_jackpot_game_choice(self, choice_id: int) -> bool:
        cursor = self.connection.execute(
            """
            UPDATE jackpot_game_choices
            SET status = 'resolved',
                resolved_at = CURRENT_TIMESTAMP
            WHERE choice_id = ? AND status = 'open'
            """,
            (choice_id,),
        )
        self.connection.commit()
    ...
    def create_mine_round(
        ...
        prize_steps: list[int],
        start_prize_stars: int,
        increment_stars: int,
    ) -> int:
        self.connection.execute(
            """
            INSERT INTO mine_rounds (
                chat_id,
                user_id,
                button_count,
                mine_count,
                mine_positions_json,
                opened_positions_json,
                prize_steps_json,
                current_prize_stars,
                start_prize_stars,
                increment_stars
            ...
            (
                ...
                button_count,
                mine_count,
                json.dumps(sorted(mine_positions)),
                json.dumps([int(step) for step in prize_steps]),
                start_prize_stars,
                increment_stars,
            ),
        )
    ...
    def get_mine_round(self, round_id: int) -> sqlite3.Row | None:
        return self.connection.execute(
            """
            SELECT *
            FROM mine_rounds
    ...
    def set_mine_round_message_id(self, round_id: int, message_id: int) -> None:
        self.connection.execute(
            """
            UPDATE mine_rounds
            SET message_id = ?
    ...
    def get_open_mine_round(self, chat_id: int, user_id: int) -> sqlite3.Row | None:
        return self.connection.execute(
            """
            SELECT *
            FROM mine_rounds
    ...
    def expire_open_mine_rounds(self, chat_id: int, user_id: int) -> sqlite3.Row | None:
        latest_open_round = self.get_open_mine_round(chat_id, user_id)
        if not latest_open_round:
            return None
    ...
    def open_mine_position(
        self,
        round_id: int,
        opened_positions: list[int],
        current_prize_stars: int,
        hit_mine: bool,
    ) -> bool:
        cursor = self.connection.execute(
            """
            UPDATE mine_rounds
            SET opened_positions_json = ?,
                current_prize_stars = ?,
                status = CASE WHEN ? THEN 'lost' ELSE status END,
                resolved_at = CASE WHEN ? THEN CURRENT_TIMESTAMP ELSE resolved_at END
            WHERE round_id = ? AND status = 'open'
            """,
            (
                json.dumps(sorted(opened_positions)),
                current_prize_stars,
                1 if hit_mine else 0,
                1 if hit_mine else 0,
                round_id,
            ),
        )
    ...
    def cash_out_mine_round(
        ...
        cursor = self.connection.execute(
            ...
            WHERE round_id = ? AND status = 'open'
            """,
            (json.dumps(sorted(opened_positions)), current_prize_stars, round_id),
        )
        self.connection.commit()
        return cursor.rowcount > 0
...
def get_jackpot_button_nft_chance_denominator(
    ...
    current_button_count = max(1, int(button_count or get_jackpot_buttons_start(db)))
    fallback = current_button_count
    value = db.get_bot_setting(
    ...
    lost_buttons = max(0, get_jackpot_buttons_start(db) - current_button_count)
    adjusted_denominator = denominator - lost_buttons * get_jackpot_button_nft_chance_step(db)
    return max(fallback, adjusted_denominator)


def get_jackpot_button_nft_chance_step(db: StatsDatabase) -> int:
    value = db.get_bot_setting(
        JACKPOT_BUTTON_NFT_CHANCE_STEP_SETTING,
        DEFAULT_JACKPOT_BUTTON_NFT_CHANCE_STEP,
    )
    try:
        return max(0, int(value))
    except (TypeError, ValueError):
        return DEFAULT_JACKPOT_BUTTON_NFT_CHANCE_STEP
...
def save_jackpot_button_settings(
    ...
    if nft_chance_step is not None:
...
def get_mines_button_count(db: StatsDatabase) -> int:
    value = db.get_bot_setting(MINES_BUTTONS_SETTING, DEFAULT_MINES_BUTTONS)
    try:
        button_count = int(value)
    except (TypeError, ValueError):
        return DEFAULT_MINES_BUTTONS

    return button_count if button_count in MINES_FIELD_OPTIONS else DEFAULT_MINES_BUTTONS


def get_min_mines_for_field(button_count: int) -> int:
    if button_count not in MINES_FIELD_OPTIONS:
        raise ValueError("Поле должно содержать 4, 9 или 16 клеток.")
    return max(MINES_MIN_COUNT, (button_count + 2) // 3)


def get_default_mines_for_field(button_count: int) -> int:
    if button_count not in MINES_FIELD_OPTIONS:
        raise ValueError("Поле должно содержать 4, 9 или 16 клеток.")
    return max(get_min_mines_for_field(button_count), (button_count + 1) // 2)


def get_mines_count(db: StatsDatabase) -> int:
    button_count = get_mines_button_count(db)
    minimum = get_min_mines_for_field(button_count)
    value = db.get_bot_setting(MINES_COUNT_SETTING, DEFAULT_MINES_COUNT)
    try:
        return max(minimum, min(int(value), button_count - 1))
    except (TypeError, ValueError):
        return get_default_mines_for_field(button_count)


def get_mines_start_stars(db: StatsDatabase) -> int:
    value = db.get_bot_setting(MINES_START_STARS_SETTING, DEFAULT_MINES_START_STARS)
    try:
        return max(1, int(value))
    except (TypeError, ValueError):
        return DEFAULT_MINES_START_STARS


def get_mines_increment_stars(db: StatsDatabase) -> int:
    value = db.get_bot_setting(
        MINES_INCREMENT_STARS_SETTING,
        DEFAULT_MINES_INCREMENT_STARS,
    )
    try:
        return max(1, int(value))
    except (TypeError, ValueError):
        return DEFAULT_MINES_INCREMENT_STARS


def validate_mines_variant(button_count: int, mine_count: int) -> None:
    if button_count not in MINES_FIELD_OPTIONS:
        raise ValueError("Поле должно содержать 4, 9 или 16 клеток.")
    minimum = get_min_mines_for_field(button_count)
    if mine_count < minimum:
        raise ValueError(
            f"Для поля {button_count} минимум {minimum} мин "
            "(треть клеток с округлением вверх)."
        )
    if mine_count >= button_count:
        raise ValueError("Мин должно быть меньше, чем кнопок.")


def mines_variant_key(button_count: int, mine_count: int) -> str:
    validate_mines_variant(button_count, mine_count)
    return f"{button_count}x{mine_count}"


def mines_prize_steps_setting_key(button_count: int, mine_count: int) -> str:
    return f"{MINES_PRIZE_STEPS_PREFIX}{mines_variant_key(button_count, mine_count)}"


def build_default_mines_prize_steps(
    db: StatsDatabase,
    button_count: int | None = None,
    mine_count: int | None = None,
) -> list[int]:
    button_count = button_count or get_mines_button_count(db)
    mine_count = mine_count or get_mines_count(db)
    validate_mines_variant(button_count, mine_count)
    safe_count = button_count - mine_count
    start = get_mines_start_stars(db)
    increment = get_mines_increment_stars(db)
    return [
        calculate_mine_prize(start, increment, step_number)
        for step_number in range(1, safe_count + 1)
    ]


def parse_mines_prize_steps(value: str) -> list[int]:
    steps = [int(number) for number in re.findall(r"\d+", value)]
    if not steps:
        raise ValueError("Укажи призы через запятую. Например: 15,20,30,45")
    if any(step < 1 for step in steps):
        raise ValueError("Каждый шаг должен быть 1⭐ или выше.")
    if any(steps[index] <= steps[index - 1] for index in range(1, len(steps))):
        raise ValueError("Каждый следующий шаг должен быть больше предыдущего.")
    return steps


def parse_mines_variation_bulk(
    payload: str,
) -> list[tuple[int, int, list[int]]]:
    parsed_variations: list[tuple[int, int, list[int]]] = []
    errors: list[str] = []
    ...
    for line_number, raw_line in enumerate(payload.splitlines(), start=1):
        ...
        if len(numbers) < 3:
            errors.append(f"строка {line_number}: нужно минимум клетки, мины и призы")
            continue

        button_count, mine_count = numbers[0], numbers[1]
        prize_steps = numbers[2:]
        try:
            validate_mines_variant(button_count, mine_count)
            expected_steps = button_count - mine_count
            if len(prize_steps) != expected_steps:
                raise ValueError(
                    f"нужно {expected_steps} призов, сейчас {len(prize_steps)}"
                )
            parse_mines_prize_steps(",".join(str(step) for step in prize_steps))
        except ValueError as error:
            errors.append(f"строка {line_number}: {error}")
            continue

        parsed_variations.append((button_count, mine_count, prize_steps))

    if errors:
        raise ValueError("Ошибки в bulk-настройке:\n" + "\n".join(errors[:12]))

    if not parsed_variations:
        raise ValueError(
            "Не нашел вариации. Пример строки: 9 3 10,18,30,50,80,120"
        )

    return parsed_variations


def normalize_steps_setting(raw_value: object) -> list[int]:
    if isinstance(raw_value, list):
        try:
            return [int(step) for step in raw_value]
        except (TypeError, ValueError):
            return []
    if isinstance(raw_value, str) and raw_value:
        ...
        if isinstance(decoded, list):
            try:
                return [int(step) for step in decoded]
            except (TypeError, ValueError):
                return []
    return []


def get_mines_prize_steps(
    db: StatsDatabase,
    button_count: int | None = None,
    mine_count: int | None = None,
) -> list[int]:
    ...
    raw_value = db.get_bot_setting(
        mines_prize_steps_setting_key(button_count, mine_count),
        [],
    )
    steps = normalize_steps_setting(raw_value)
    if len(steps) == safe_count and all(step > 0 for step in steps):
        return steps

    if (
        button_count == get_mines_button_count(db)
        and mine_count == get_mines_count(db)
    ):
        raw_legacy_value = db.get_bot_setting(MINES_PRIZE_STEPS_SETTING, [])
        legacy_steps = normalize_steps_setting(raw_legacy_value)
        if len(legacy_steps) == safe_count and all(step > 0 for step in legacy_steps):
            return legacy_steps

    return build_default_mines_prize_steps(db, button_count, mine_count)


def get_mines_prize_steps_text(
    db: StatsDatabase,
    button_count: int | None = None,
    mine_count: int | None = None,
) -> str:
    return ", ".join(
        f"{step}⭐"
        for step in get_mines_prize_steps(db, button_count, mine_count)
    )


def get_mines_variations(db: StatsDatabase) -> list[tuple[int, int]]:
    raw_value = db.get_bot_setting(MINES_VARIATIONS_SETTING, [])
    variations: set[tuple[int, int]] = set()
    if isinstance(raw_value, list):
        for item in raw_value:
            match = re.fullmatch(r"\s*(\d+)x(\d+)\s*", str(item))
            if not match:
                continue
            button_count, mine_count = int(match.group(1)), int(match.group(2))
            try:
                validate_mines_variant(button_count, mine_count)
            except ValueError:
                continue
            variations.add((button_count, mine_count))

    variations.add((get_mines_button_count(db), get_mines_count(db)))
    return sorted(variations)


def save_mines_variations(db: StatsDatabase, variations: set[tuple[int, int]]) -> None:
    db.set_bot_setting(
        MINES_VARIATIONS_SETTING,
        [mines_variant_key(button_count, mine_count) for button_count, mine_count in sorted(variations)],
    )


def add_mines_variation(db: StatsDatabase, button_count: int, mine_count: int) -> None:
    validate_mines_variant(button_count, mine_count)
    variations = set(get_mines_variations(db))
    variations.add((button_count, mine_count))
    save_mines_variations(db, variations)


def save_mines_prize_steps(
    db: StatsDatabase,
    steps: list[int],
    button_count: int | None = None,
    mine_count: int | None = None,
    ...
    validate_mines_variant(button_count, mine_count)
    safe_count = button_count - mine_count
    if len(steps) != safe_count:
        raise ValueError(
            f"Нужно указать ровно {safe_count} шагов: кнопок {button_count} "
            f"минус мин {mine_count}."
        )
    if any(step < 1 for step in steps):
        raise ValueError("Каждый шаг должен быть 1⭐ или выше.")
    if any(steps[index] <= steps[index - 1] for index in range(1, len(steps))):
    ...
    add_mines_variation(db, button_count, mine_count)
    db.set_bot_setting(mines_prize_steps_setting_key(button_count, mine_count), steps)
    if button_count == get_mines_button_count(db) and mine_count == get_mines_count(db):
        db.set_bot_setting(MINES_PRIZE_STEPS_SETTING, steps)
        db.set_bot_setting(MINES_START_STARS_SETTING, steps[0])
        fallback_increment = steps[1] - steps[0] if len(steps) > 1 else get_mines_increment_stars(db)
        db.set_bot_setting(MINES_INCREMENT_STARS_SETTING, max(1, fallback_increment))


def save_mines_settings(
    db: StatsDatabase,
    button_count: int | None = None,
    mine_count: int | None = None,
    start_stars: int | None = None,
    increment_stars: int | None = None,
) -> None:
    if button_count is not None:
        if button_count not in MINES_FIELD_OPTIONS:
            raise ValueError("Поле должно содержать 4, 9 или 16 клеток.")
        db.set_bot_setting(MINES_BUTTONS_SETTING, button_count)
        if get_mines_count(db) >= button_count:
            db.set_bot_setting(MINES_COUNT_SETTING, button_count - 1)

    if mine_count is not None:
        current_button_count = get_mines_button_count(db)
        validate_mines_variant(current_button_count, mine_count)
        db.set_bot_setting(MINES_COUNT_SETTING, mine_count)

    if start_stars is not None:
        if start_stars < 1:
            raise ValueError("Стартовый выигрыш должен быть 1⭐ или выше.")
        db.set_bot_setting(MINES_START_STARS_SETTING, start_stars)

    if increment_stars is not None:
        if increment_stars < 1:
            raise ValueError("Прибавка должна быть 1⭐ или выше.")
        db.set_bot_setting(MINES_INCREMENT_STARS_SETTING, increment_stars)

    if start_stars is not None or increment_stars is not None:
        db.set_bot_setting(MINES_PRIZE_STEPS_SETTING, "")


def get_service_commission_percent(db: StatsDatabase) -> float:
    value = db.get_bot_setting(
        SERVICE_COMMISSION_PERCENT_SETTING,
...
def save_service_commission_percent(db: StatsDatabase, percent: float) -> None:
    if percent < 0:
        raise ValueError("Комиссия не может быть отрицательной.")
    db.set_bot_setting(SERVICE_COMMISSION_PERCENT_SETTING, percent)
...
def format_star_amount(amount: float) -> str:
...
def parse_chance_denominator(token: str, label: str) -> int:
    normalized = token.strip().lower()
    fraction_match = re.search(r"1\s*[/\\:к]\s*(\d+)", normalized)
    if fraction_match:
...
async def render_saved_template(
    ...
    values = {
        ...
        "nft_chance_step": str(get_jackpot_button_nft_chance_step(db)),
        "mine_button_count": str(get_mines_button_count(db)),
        "mine_count": str(get_mines_count(db)),
        "mine_start_stars": str(get_mines_start_stars(db)),
        "mine_increment_stars": str(get_mines_increment_stars(db)),
        "mine_prize_steps": get_mines_prize_steps_text(db),
...
def jackpot_button_reveal_keyboard(
    ...
    display_nft_position = nft_position
    if not nft_armed and display_nft_position == selected_position:
        display_nft_position = next(
            (position for position in range(1, button_count + 1) if position != selected_position),
            nft_position,
        )
    ...
    for position in range(1, button_count + 1):
        ...
        if position == display_nft_position:
            label = "✅ NFT" if is_selected else "NFT"
        elif is_selected and show_small_prizes and selected_small_prize is not None:
            prize = selected_small_prize or random.choice(JACKPOT_BUTTON_SMALL_PRIZES)
            label = f"✅ {prize}⭐"
        elif is_selected:
            label = "✅ Пусто"
        else:
...
async def send_jackpot_button_challenge_from_message(
    trigger_message,
    chat_id: int,
    user: User,
    config: BotConfig,
    ...
    db.expire_open_jackpot_game_choices(chat_id, user.id)
    gift = await choose_owner_gift_from_api(config, db)
    if not gift:
    ...
    button_count = db.get_jackpot_button_count(user.id)
    ...
    sent_message = await trigger_message.reply_text(
        message,
        entities=deserialize_entities(entities_data) or None,
...
async def send_jackpot_button_challenge(
    ...
    await send_jackpot_button_challenge_from_message(
        update.message,
...
def mine_round_positions(row: sqlite3.Row) -> tuple[set[int], set[int]]:
    try:
        mine_positions = set(int(position) for position in json.loads(row["mine_positions_json"] or "[]"))
    except (TypeError, ValueError, json.JSONDecodeError):
        mine_positions = set()

    try:
        opened_positions = set(int(position) for position in json.loads(row["opened_positions_json"] or "[]"))
    except (TypeError, ValueError, json.JSONDecodeError):
        opened_positions = set()

    return mine_positions, opened_positions


def calculate_mine_prize(start_stars: int, increment_stars: int, safe_opened_count: int) -> int:
    if safe_opened_count <= 0:
        return 0
    return start_stars + (safe_opened_count - 1) * increment_stars


def mine_prize_steps_from_round(row: sqlite3.Row) -> list[int]:
    safe_count = int(row["button_count"]) - int(row["mine_count"])
    try:
        steps = [int(step) for step in json.loads(row["prize_steps_json"] or "[]")]
    except (TypeError, ValueError, json.JSONDecodeError):
        steps = []
    ...
    return [
        calculate_mine_prize(
            int(row["start_prize_stars"]),
            int(row["increment_stars"]),
            step_number,
        )
        for step_number in range(1, safe_count + 1)
    ]
...
def calculate_mine_prize_from_steps(prize_steps: list[int], safe_opened_count: int) -> int:
    if safe_opened_count <= 0:
        return 0
    if not prize_steps:
        return 0
    return prize_steps[min(safe_opened_count, len(prize_steps)) - 1]


def format_mine_prize_steps(prize_steps: list[int]) -> str:
    return " -> ".join(f"{step}⭐" for step in prize_steps)


def mine_keyboard(
    round_id: int,
    button_count: int,
    opened_positions: set[int],
    current_prize_stars: int,
    mine_positions: set[int] | None = None,
    status: str = "open",
    selected_position: int | None = None,
) -> InlineKeyboardMarkup:
    buttons = []
    reveal = status in {"lost", "cashed_out", "expired"}
    for position in range(1, button_count + 1):
        if reveal and mine_positions and position in mine_positions:
            label = "✅ 💣" if position == selected_position else "💣"
        elif position in opened_positions:
            label = "✅"
        elif reveal:
            label = "Пусто"
        else:
            label = "🎁"
        buttons.append(
            InlineKeyboardButton(label, callback_data=f"mine:open:{round_id}:{position}")
        )

    rows = [buttons[index:index + 3] for index in range(0, len(buttons), 3)]
    if status == "open" and current_prize_stars > 0:
        rows.append(
            [
                InlineKeyboardButton(
                    f"Забрать {current_prize_stars}⭐",
                    callback_data=f"mine:cash:{round_id}",
                )
            ]
        )
    return InlineKeyboardMarkup(rows)
...
async def build_mines_start_message(
    config: BotConfig,
    db: StatsDatabase,
    user: User,
    button_count: int,
    ...
    rendered = await render_saved_template(
        ...
        stats=stats,
        extra_values={
            "mine_button_count": str(button_count),
            "mine_count": str(mine_count),
            "mine_start_stars": str(start_stars),
            "mine_increment_stars": str(increment_stars),
            "mine_prize_steps": format_mine_prize_steps(prize_steps),
            "current_prize": "0",
            "opened_count": "0",
            "safe_count": str(button_count - mine_count),
            "remaining_safe_count": str(button_count - mine_count),
            "expired_previous": "1" if expired_previous else "0",
        },
    )
    if rendered:
    ...
    previous_text = (
        "\nПрошлая игра в минки была закрыта, потому что выпал новый 777."
        if expired_previous else ""
    )
    return (
        f"{get_user_display_name(user)} выбил 777!\n\n"
        f"Режим: минки. Кнопок: {button_count}, мин: {mine_count}.\n"
        f"Лестница призов: {format_mine_prize_steps(prize_steps)}.\n"
        "Можно открыть следующую кнопку или забрать текущий приз."
        f"{previous_text}"
    ), []


async def build_mines_safe_message(
    config: BotConfig,
    db: StatsDatabase,
    user: User,
    selected_position: int,
    ...
    rendered = await render_saved_template(
        ...
        extra_values={
            ...
            "mine_increment_stars": str(increment_stars),
            "mine_prize_steps": format_mine_prize_steps(prize_steps),
            "current_prize": str(current_prize),
            "opened_count": str(opened_count),
            "safe_count": str(safe_count),
            "remaining_safe_count": str(max(0, safe_count - opened_count)),
        },
    )
    if rendered:
        text, entities_data, _, _ = rendered
    ...
    return (
        f"{get_user_display_name(user)} открыл кнопку #{selected_position}.\n\n"
        f"Безопасно. Текущий приз: {current_prize}⭐.\n"
        "Можно открыть следующую кнопку или забрать приз."
    ), []


async def build_mines_cashout_message(
    config: BotConfig,
    db: StatsDatabase,
    user: User,
    current_prize: int,
    ...
    rendered = await render_saved_template(
        ...
        extra_values={
            ...
            "mine_prize_steps": format_mine_prize_steps(prize_steps),
            "current_prize": str(current_prize),
            "stars": str(current_prize),
            "amount": str(current_prize),
            "stars_balance": str(stars_balance),
            "balance": str(stars_balance),
            "opened_count": str(opened_count),
            "safe_count": str(safe_count),
            "remaining_safe_count": str(max(0, safe_count - opened_count)),
        },
    ...
    return (
        f"{get_user_display_name(user)} забрал {current_prize}⭐ в минках.\n\n"
        f"Stars-баланс: {stars_balance}⭐.\n"
        f"Вывод доступен от {MIN_STARS_WITHDRAW}⭐: /withdraw"
    ), []


async def build_mines_lost_message(
    config: BotConfig,
    db: StatsDatabase,
    user: User,
    selected_position: int,
    ...
    return (
        f"{get_user_display_name(user)} открыл кнопку #{selected_position} и попал на мину.\n\n"
        f"Было на кону: {lost_prize}⭐.\n"
        "Приз сгорел."
    ), []


async def send_mines_challenge_from_message(
    trigger_message,
    chat_id: int,
    user: User,
    config: BotConfig,
    ...
    db.expire_open_jackpot_game_choices(chat_id, user.id)
    expired_round = db.expire_open_mine_rounds(
        chat_id,
        user.id,
    )
    button_count = button_count or get_mines_button_count(db)
    mine_count = mine_count or get_mines_count(db)
    validate_mines_variant(button_count, mine_count)
    prize_steps = get_mines_prize_steps(db, button_count, mine_count)
    start_stars = prize_steps[0] if prize_steps else get_mines_start_stars(db)
    increment_stars = (
        prize_steps[1] - prize_steps[0]
        if len(prize_steps) > 1
        else get_mines_increment_stars(db)
    )
    mine_positions = random.sample(range(1, button_count + 1), mine_count)
    round_id = db.create_mine_round(
        chat_id,
        user.id,
        button_count,
        mine_count,
    ...
    message, entities_data = await build_mines_start_message(
        config,
        db,
        user,
        button_count,
    ...
    sent_message = await trigger_message.reply_text(
        message,
        entities=deserialize_entities(entities_data) or None,
        reply_markup=mine_keyboard(round_id, button_count, set(), 0),
    )
    db.set_mine_round_message_id(round_id, sent_message.message_id)


async def send_mines_challenge(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    config: BotConfig,
    db: StatsDatabase,
    ...
    await send_mines_challenge_from_message(
        update.message,
        update.effective_chat.id,
        update.effective_user,
        config,
...
async def handle_mine_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not query.from_user or not query.data or not query.message:
        return
    ...
    config: BotConfig = context.application.bot_data["config"]
    db: StatsDatabase = context.application.bot_data["db"]
    round_row = db.get_mine_round(round_id)
    if not round_row:
        await query.answer("Раунд не найден.", show_alert=True)
        return

    if int(round_row["user_id"]) != query.from_user.id:
        await query.answer("Это не твоя игра в минки.", show_alert=True)
        return

    status = str(round_row["status"])
    if status != "open":
        if status == "expired":
            await query.answer("Эта игра уже закрыта, потому что выпал новый 777.", show_alert=True)
        elif status == "lost":
            await query.answer("Эта игра уже проиграна.", show_alert=True)
        else:
            await query.answer("Этот приз уже забран.", show_alert=True)
        return

    mine_positions, opened_positions = mine_round_positions(round_row)
    button_count = int(round_row["button_count"])
    mine_count = int(round_row["mine_count"])
    prize_steps = mine_prize_steps_from_round(round_row)
    current_prize = int(round_row["current_prize_stars"] or 0)
    user_stats = db.get_user_stats(int(round_row["chat_id"]), query.from_user.id)

    if action == "cash":
        if current_prize <= 0:
            await query.answer("Сначала открой безопасную кнопку.", show_alert=True)
            return
        if not db.cash_out_mine_round(round_id, sorted(opened_positions), current_prize):
            await query.answer("Этот раунд уже завершен.", show_alert=True)
            return
        stars_balance = db.add_stars_balance(query.from_user.id, current_prize)
        db.record_prize_economy(
            int(round_row["chat_id"]),
            query.from_user.id,
            current_prize,
        ...
        message, entities_data = await build_mines_cashout_message(
            config,
            db,
            query.from_user,
            current_prize,
            stars_balance,
            len(opened_positions),
            button_count,
            mine_count,
            int(round_row["start_prize_stars"]),
            int(round_row["increment_stars"]),
            prize_steps,
            stats=user_stats,
        )
        await query.answer(f"+{current_prize}⭐ на баланс.", show_alert=True)
        await query.message.edit_text(
            message,
            entities=deserialize_entities(entities_data) or None,
    ...
    if selected_position < 1 or selected_position > button_count:
        await query.answer()
        return

    if selected_position in opened_positions:
        await query.answer("Эта кнопка уже открыта.", show_alert=True)
        return

    new_opened_positions = set(opened_positions)
    new_opened_positions.add(selected_position)
    hit_mine = selected_position in mine_positions
    if hit_mine:
        lost_prize = current_prize
        if not db.open_mine_position(round_id, sorted(new_opened_positions), 0, True):
            await query.answer("Этот раунд уже завершен.", show_alert=True)
            return
        message, entities_data = await build_mines_lost_message(
            config,
            db,
            query.from_user,
            selected_position,
        ...
        await query.answer("Мина. Приз сгорел.", show_alert=True)
        await query.message.edit_text(
            message,
            entities=deserialize_entities(entities_data) or None,
            reply_markup=mine_keyboard(
    ...
    safe_opened_count = len(new_opened_positions - mine_positions)
    new_prize = calculate_mine_prize_from_steps(prize_steps, safe_opened_count)
    if not db.open_mine_position(round_id, sorted(new_opened_positions), new_prize, False):
        await query.answer("Этот раунд уже завершен.", show_alert=True)
        return

    safe_count = button_count - mine_count
    if safe_opened_count >= safe_count:
        if not db.cash_out_mine_round(round_id, sorted(new_opened_positions), new_prize):
            await query.answer("Этот раунд уже завершен.", show_alert=True)
            return
        stars_balance = db.add_stars_balance(query.from_user.id, new_prize)
        db.record_prize_economy(
            int(round_row["chat_id"]),
            query.from_user.id,
            new_prize,
        ...
        await query.answer(f"Все безопасные кнопки открыты. +{new_prize}⭐", show_alert=True)
        await query.message.edit_text(
            message,
            entities=deserialize_entities(entities_data) or None,
            reply_markup=mine_keyboard(
    ...
    message, entities_data = await build_mines_safe_message(
        config,
        db,
        query.from_user,
        selected_position,
    ...
    await query.answer(f"Безопасно. Сейчас {new_prize}⭐.", show_alert=True)
    await query.message.edit_text(
        message,
        entities=deserialize_entities(entities_data) or None,
        reply_markup=mine_keyboard(
...
def jackpot_game_choice_keyboard(choice_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "Кейсы",
                    callback_data=f"jpchoose:{choice_id}:cases",
                ),
                InlineKeyboardButton(
                    "Минки",
                    callback_data=f"jpchoose:{choice_id}:mines",
                ),
            ]
        ]
    )
...
def mines_field_keyboard(choice_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "2x2 · 4 клетки",
                    callback_data=f"minefield:{choice_id}:4",
                ),
                InlineKeyboardButton(
                    "3x3 · 9 клеток",
                    callback_data=f"minefield:{choice_id}:9",
                ),
            ],
            [
                InlineKeyboardButton(
                    "4x4 · 16 клеток",
                    callback_data=f"minefield:{choice_id}:16",
                )
            ],
        ]
    )
...
def mines_field_side(button_count: int) -> int:
    return {4: 2, 9: 3, 16: 4}[button_count]


def mines_setup_text(db: StatsDatabase, button_count: int, mine_count: int) -> str:
    minimum = get_min_mines_for_field(button_count)
    maximum = button_count - 1
    safe_count = button_count - mine_count
    return (
        "💣 Минки\n\n"
        f"Поле: {mines_field_side(button_count)}x{mines_field_side(button_count)} "
        f"({button_count} клеток)\n"
        f"💣 Мин: {mine_count}\n"
        f"✅ Безопасных клеток: {safe_count}\n"
        f"Диапазон: {minimum}-{maximum}\n\n"
        f"⭐ Лестница: {get_mines_prize_steps_text(db, button_count, mine_count)}"
    )
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "➖ 1 мина",
                    callback_data=f"mineset:{choice_id}:{button_count}:{mine_count}:dec",
                ),
                InlineKeyboardButton(
                    "➕ 1 мина",
                    callback_data=f"mineset:{choice_id}:{button_count}:{mine_count}:inc",
                ),
            ],
            [
                InlineKeyboardButton(
                    f"▶️ Начать · {mine_count} мин",
                    callback_data=f"mineset:{choice_id}:{button_count}:{mine_count}:play",
                )
            ],
            [
                InlineKeyboardButton(
                    "↩️ Выбрать другое поле",
                    callback_data=f"mineset:{choice_id}:{button_count}:{mine_count}:fields",
                )
            ],
        ]
    )
...
async def send_jackpot_game_choice(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> None:
    if not update.effective_chat or not update.effective_user or not update.message:
    ...
    db: StatsDatabase = context.application.bot_data["db"]
    db.expire_open_jackpot_game_choices(update.effective_chat.id, update.effective_user.id)
    choice_id = db.create_jackpot_game_choice(update.effective_chat.id, update.effective_user.id)
    sent_message = await update.message.reply_text(
        f"{get_user_display_name(update.effective_user)} выбил 777.\n\n"
        "Выбери, во что сыграть:",
        reply_markup=jackpot_game_choice_keyboard(choice_id),
    )
    db.set_jackpot_game_choice_message_id(choice_id, sent_message.message_id)


async def send_mines_variation_choice(
    trigger_message,
    chat_id: int,
    user: User,
    db: StatsDatabase,
    ...
    if choice_id is None:
        db.expire_open_jackpot_game_choices(chat_id, user.id)
        choice_id = db.create_jackpot_game_choice(chat_id, user.id)

    sent_message = await trigger_message.reply_text(
        "Минки\n\nСначала выбери размер поля:",
        reply_markup=mines_field_keyboard(choice_id),
    )
    db.set_jackpot_game_choice_message_id(choice_id, sent_message.message_id)


async def handle_jackpot_game_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not query.from_user or not query.data or not query.message:
        return
    ...
    config: BotConfig = context.application.bot_data["config"]
    db: StatsDatabase = context.application.bot_data["db"]
    choice = db.get_jackpot_game_choice(choice_id)
    if not choice:
        await query.answer("Выбор не найден.", show_alert=True)
        return
    if choice["status"] != "open":
        await query.answer("Этот выбор уже закрыт.", show_alert=True)
        return
    if int(choice["user_id"]) != query.from_user.id:
        await query.answer("Это не твой 777.", show_alert=True)
        return
    ...
    stats = db.get_user_stats(chat_id, query.from_user.id)
    if action == "cases":
        if not db.resolve_jackpot_game_choice(choice_id):
            await query.answer("Этот выбор уже закрыт.", show_alert=True)
            return
        await query.answer("Выбраны кейсы.", show_alert=True)
        try:
            await query.message.edit_text("Выбраны кейсы.")
        except TelegramError:
            pass
        await send_jackpot_button_challenge_from_message(
            query.message,
    ...
    if action == "mines":
        await query.answer("Выбери поле.", show_alert=True)
        try:
            await query.message.edit_text(
                "Минки\n\nСначала выбери размер поля:",
                reply_markup=mines_field_keyboard(choice_id),
...
async def handle_mines_variation_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query or not query.from_user or not query.data or not query.message:
        return
    ...
    if callback_type == "minecfg":
        await query.answer("Меню минок обновлено. Выбери поле.", show_alert=True)
        await query.message.edit_text(
            "Минки\n\nСначала выбери размер поля:",
            reply_markup=mines_field_keyboard(choice_id),
        )
    ...
    if callback_type == "minefield":
        if len(parts) != 3:
            await query.answer()
            return
        try:
            button_count = int(parts[2])
            mine_count = get_default_mines_for_field(button_count)
            validate_mines_variant(button_count, mine_count)
        except ValueError as error:
            await query.answer(str(error), show_alert=True)
            return

        await query.answer(f"Поле {mines_field_side(button_count)}x{mines_field_side(button_count)}.")
        await query.message.edit_text(
            mines_setup_text(db, button_count, mine_count),
            reply_markup=mines_count_keyboard(choice_id, button_count, mine_count),
        )
        return

    if callback_type != "mineset" or len(parts) != 5:
        await query.answer()
        return
    ...
    try:
        ...
        await query.answer("Некорректная настройка минок.", show_alert=True)
        return
    action = parts[4]

    if action == "fields":
        await query.answer("Выбери поле.")
        await query.message.edit_text(
            "Минки\n\nСначала выбери размер поля:",
            reply_markup=mines_field_keyboard(choice_id),
        )
    ...
    minimum = get_min_mines_for_field(button_count)
    maximum = button_count - 1
    if action in {"dec", "inc"}:
        new_mine_count = mine_count - 1 if action == "dec" else mine_count + 1
        if new_mine_count < minimum:
            await query.answer(f"Минимум для этого поля: {minimum} мин.", show_alert=True)
            return
        if new_mine_count > maximum:
            await query.answer(f"Максимум для этого поля: {maximum} мин.", show_alert=True)
            return

        await query.answer(f"Теперь мин: {new_mine_count}.")
        await query.message.edit_text(
            mines_setup_text(db, button_count, new_mine_count),
            reply_markup=mines_count_keyboard(choice_id, button_count, new_mine_count),
        )
        return
    ...
    stats = db.get_user_stats(chat_id, query.from_user.id)
    await query.answer(f"Запуск: {button_count} клеток, {mine_count} мин.", show_alert=True)
    try:
        await query.message.edit_text(
            f"Минки запущены: {mines_field_side(button_count)}x{mines_field_side(button_count)}, "
            f"мин {mine_count}."
        )
    except TelegramError:
        pass
...
async def set_message_template(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "nft_chance/nft_chance_denominator/nft_chance_step - шанс NFT в 777-кнопках\n"
...
def game_settings_text(db: StatsDatabase) -> str:
    ...
    rank_gain = get_rank_points_per_spin(db)
    start_buttons = get_jackpot_buttons_start(db)
    min_buttons = get_jackpot_buttons_min(db)
    ...
    variations_text = "\n".join(
        f"- {button_count} клеток / {mine_count} мин: "
        f"{get_mines_prize_steps_text(db, button_count, mine_count)}"
        for button_count, mine_count in displayed_variations
    ) or "нет"
    hidden_variations_count = max(0, len(variations) - len(displayed_variations))
    if hidden_variations_count:
        variations_text += f"\n...и еще {hidden_variations_count} вариаций"
    return (
        "Настройки игры\n\n"
        f"Режим: {game_mode_label(mode)}\n"
        f"Цена прокрута: {spin_price}⭐\n"
        f"Очки ранга за прокрут: +{format_rank_points(rank_gain)}\n\n"
        f"777-кнопки: старт {start_buttons}, "
        f"минимум {min_buttons}, "
        f"минус {get_jackpot_buttons_decrease(db)} за промах.\n"
        f"Шанс NFT: 1/{get_jackpot_button_nft_chance_denominator(db, start_buttons)}"
        f" -> 1/{get_jackpot_button_nft_chance_denominator(db, min_buttons)} при минимуме кнопок\n"
        f"Шаг NFT: -{nft_chance_step} к знаменателю за каждую потерянную кнопку\n"
        f"15/25⭐ в коробках: от {get_jackpot_button_stars_min_price(db)}⭐ за прокрут.\n\n"
        f"Минки: {mines_buttons} кнопок, {mines_count} мин, "
        f"старт {get_mines_start_stars(db)}⭐, "
        ...
        "Много вариаций: /game minesbulk и дальше строки с вариациями\n"
        "Комиссия: /game commission 15"
    )
...
def game_settings_keyboard(db: StatsDatabase) -> InlineKeyboardMarkup:
    ...
    mode_buttons = [
        ...
        InlineKeyboardButton(
            f"{'✓ ' if mode == GAME_MODE_MINES else ''}Минки",
            callback_data=f"game:mode:{GAME_MODE_MINES}",
        ),
        InlineKeyboardButton(
...
def economy_text(config: BotConfig, db: StatsDatabase) -> str:
    totals = db.get_economy_totals(config.allowed_chat_ids)
    spent_total = float(totals["spent_total"] or 0)
    service_commission = float(totals["service_commission"] or 0)
    net_income = float(totals["net_income"] or 0)
    prizes_won = float(totals["prizes_won"] or 0)
    profit = net_income - prizes_won
    return (
        "Экономика\n\n"
        f"Платных спинов: {int(totals['paid_spins'] or 0)}\n"
        f"Потратили игроки: {format_star_amount(spent_total)}⭐\n"
        f"Комиссия сервиса: {format_star_amount(service_commission)}⭐ "
        f"({get_service_commission_percent(db):g}%)\n"
        f"Чистая получка: {format_star_amount(net_income)}⭐\n"
        f"Выиграли/выдано: {format_star_amount(prizes_won)}⭐\n"
        f"Событий выдачи: {int(totals['prize_events'] or 0)}\n"
        f"Итог: {format_star_amount(profit)}⭐"
    )


async def show_economy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat or not update.effective_user or not update.message:
        return
    ...
    await update.message.reply_text(economy_text(config, db))
...
async def manage_game_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args:
        ...
        if action in {"classic", "old", "классика", "обычный"} and len(context.args) == 1:
        ...
        elif action in {"mines", "mine", "минки"} and len(context.args) == 1:
            ...
            if requested_mode in {"classic", "old", "классика", "обычный"}:
                ...
                db.set_bot_setting(GAME_MODE_SETTING, GAME_MODE_JACKPOT_BUTTONS)
            elif requested_mode in {"mines", "mine", "минки"}:
                db.set_bot_setting(GAME_MODE_SETTING, GAME_MODE_MINES)
            ...
            try:
                if subaction in {"buttons", "button", "boxes", "box", "кнопки"}:
                    value = parse_integer_from_token(value_text, "количество кнопок в минках")
                    save_mines_settings(db, button_count=value)
                elif subaction in {"count", "mines", "mine", "мины", "мин"}:
                    value = parse_integer_from_token(value_text, "количество мин")
                    save_mines_settings(db, mine_count=value)
                elif subaction in {"start", "base", "старт", "начало"}:
                    value = parse_integer_from_token(value_text, "стартовый выигрыш минок")
                    save_mines_settings(db, start_stars=value)
                elif subaction in {"step", "inc", "increment", "рост", "шаг", "плюс"}:
                    value = parse_integer_from_token(value_text, "прибавка минок")
                    save_mines_settings(db, increment_stars=value)
                elif subaction in {"variant", "add", "field", "поле", "вариант"}:
                    numbers = [int(number) for number in re.findall(r"\d+", value_text)]
                    if len(numbers) < 2:
                        raise ValueError("Формат: /game mines variant 9 3")
                    add_mines_variation(db, numbers[0], numbers[1])
                elif subaction in {"bulk", "many", "list", "multi", "пакет", "список", "много"}:
                    payload = extract_text_payload_after_prefix(update.message.text or "", 3)
                    bulk_text = payload[0] if payload else value_text
                    saved_count = 0
                    for button_count, mine_count, prize_steps in parse_mines_variation_bulk(bulk_text):
                        save_mines_prize_steps(db, prize_steps, button_count, mine_count)
                        saved_count += 1
                    await update.message.reply_text(
                        f"Bulk-настройка минок сохранена: {saved_count} вариаций.\n\n"
                        + game_settings_text(db)
                    )
                    return
                elif subaction in {"steps", "ladder", "prizes", "лестница", "призы"}:
                    numbers = [int(number) for number in re.findall(r"\d+", value_text)]
                    if len(numbers) >= 4:
                        button_count, mine_count = numbers[0], numbers[1]
                        try:
                            ...
                            button_count = mine_count = 0
                        if button_count and mine_count and len(numbers[2:]) == button_count - mine_count:
                            save_mines_prize_steps(db, numbers[2:], button_count, mine_count)
                        else:
                            save_mines_prize_steps(db, parse_mines_prize_steps(value_text))
                    else:
                        save_mines_prize_steps(db, parse_mines_prize_steps(value_text))
                else:
                    await update.message.reply_text(
                        "Формат: /game mines buttons 9, /game mines count 3, "
                        "/game mines start 15, /game mines step 5, "
                        "/game mines variant 9 3, /game mines steps 9 3 10,18,30,50,80,120"
                    )
                    return
            except ValueError as error:
                await update.message.reply_text(str(error))
            ...
            try:
                denominator = parse_chance_denominator(" ".join(context.args[1:]), "шанс NFT 1/N")
                save_jackpot_button_settings(db, nft_chance_denominator=denominator)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"nftstep", "stepnft", "nft_step", "шагнфт", "нфтшаг"} and len(context.args) >= 2:
            try:
                nft_step = parse_integer_from_token(context.args[1], "шаг NFT")
                save_jackpot_button_settings(db, nft_chance_step=nft_step)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minesbuttons", "minebuttons", "minesboxes", "mineboxes", "минкикнопки"} and len(context.args) >= 2:
            try:
                button_count = parse_integer_from_token(context.args[1], "количество кнопок в минках")
                save_mines_settings(db, button_count=button_count)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minescount", "minecount", "minesmines", "minecount", "минкимины"} and len(context.args) >= 2:
            try:
                mine_count = parse_integer_from_token(context.args[1], "количество мин")
                save_mines_settings(db, mine_count=mine_count)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minesstart", "minestart", "minesbase", "минкистарт"} and len(context.args) >= 2:
            try:
                start_stars = parse_integer_from_token(context.args[1], "стартовый выигрыш минок")
                save_mines_settings(db, start_stars=start_stars)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minesstep", "minestep", "minesinc", "mineincrement", "минкишаг"} and len(context.args) >= 2:
            try:
                increment_stars = parse_integer_from_token(context.args[1], "прибавка минок")
                save_mines_settings(db, increment_stars=increment_stars)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minessteps", "minesteps", "minesladder", "mineprizes", "минкилестница"} and len(context.args) >= 2:
            try:
                numbers = [int(number) for number in re.findall(r"\d+", " ".join(context.args[1:]))]
                if len(numbers) >= 4:
                    button_count, mine_count = numbers[0], numbers[1]
                    try:
                        validate_mines_variant(button_count, mine_count)
                    ...
                    if button_count and mine_count and len(numbers[2:]) == button_count - mine_count:
                        ...
                        save_mines_prize_steps(db, parse_mines_prize_steps(" ".join(context.args[1:])))
                else:
                    save_mines_prize_steps(db, parse_mines_prize_steps(" ".join(context.args[1:])))
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minesvariant", "minevariant", "minesadd", "mineadd", "минкивариант"} and len(context.args) >= 3:
            try:
                button_count = parse_integer_from_token(context.args[1], "кнопки варианта минок")
                mine_count = parse_integer_from_token(context.args[2], "мины варианта минок")
                add_mines_variation(db, button_count, mine_count)
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {"minesbulk", "minebulk", "mineslist", "minebulksteps", "минкипакет"}:
            payload = extract_text_payload_after_prefix(update.message.text or "", 2)
            if not payload:
                await update.message.reply_text(
                    ...
                    "/game minesbulk\n"
                    "4 2 10,25\n"
                    "9 3 10,18,30,50,80,120\n"
                    "16 6 20,30,45,65,90,120,160,210,270,350"
                )
                return
            try:
                saved_count = 0
                for button_count, mine_count, prize_steps in parse_mines_variation_bulk(payload[0]):
                    save_mines_prize_steps(db, prize_steps, button_count, mine_count)
                    saved_count += 1
            except ValueError as error:
                await update.message.reply_text(str(error))
            ...
            return
        elif action in {"commission", "fee", "servicefee", "комиссия"} and len(context.args) >= 2:
            try:
                match = re.search(r"\d+(?:[.,]\d+)?", " ".join(context.args[1:]))
                if not match:
                    raise ValueError("не нашел число в поле: комиссия")
                save_service_commission_percent(db, float(match.group().replace(",", ".")))
            except ValueError as error:
                await update.message.reply_text(str(error))
                return
        elif action in {
            ...
            await update.message.reply_text(
                ...
                "/game minesstart 15\n"
                "/game minesstep 5\n"
                "/game minesvariant 9 3\n"
                "/game minessteps 9 3 10,18,30,50,80,120\n"
                "/game minesbulk\n"
                "4 2 10,25\n"
                "9 3 10,18,30,50,80,120\n"
                "16 6 20,30,45,65,90,120,160,210,270,350\n"
...
def owner_panel_status_text(config: BotConfig, db: StatsDatabase) -> str:
    ...
    known_chats = db.get_known_chats(config.allowed_chat_ids)
    economy_totals = db.get_economy_totals(config.allowed_chat_ids)
    economy_profit = float(economy_totals["net_income"] or 0) - float(economy_totals["prizes_won"] or 0)
    ...
    return (
        ...
        f"(шаг -{get_jackpot_button_nft_chance_step(db)})\n"
        f"15/25⭐ в коробках: от {get_jackpot_button_stars_min_price(db)}⭐ за прокрут\n"
        f"Минки: {get_mines_button_count(db)} кнопок, {get_mines_count(db)} мин, "
        f"старт {get_mines_start_stars(db)}⭐, "
        f"+{get_mines_increment_stars(db)}⭐ за безопасную кнопку\n\n"
        f"Экономика: получка {format_star_amount(float(economy_totals['net_income'] or 0))}⭐, "
        f"выдача {format_star_amount(float(economy_totals['prizes_won'] or 0))}⭐, "
        f"итог {format_star_amount(economy_profit)}⭐\n\n"
...
def owner_panel_commands_text() -> str:
    return (
        ...
        "/owner или /panel\n\n"
        "Игра:\n"
        "/game, /game mode classic, /game mode buttons, /game mode mines, /game mode choice, /game price 37, "
        "/game buttons 12, /game minbuttons 4, /game decrease 1, "
        "/game nftchance 1/18, /game nftstep 4, /game minesbuttons 9, "
        "/game minescount 3, /game minesvariant 9 3, "
        "/game minessteps 9 3 10,18,30,50,80,120, /game minesbulk, "
        "/game commission 15, /economy\n\n"
        "Тексты:\n"
...
async def react_to_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    db.record_spin_economy(
        update.effective_chat.id,
        update.effective_user.id,
        get_spin_price_stars(db),
        get_service_commission_percent(db),
        result,
    )
    ...
    if result == "jackpot":
        game_mode = get_game_mode(db)
        if game_mode == GAME_MODE_JACKPOT_BUTTONS:
        ...
        elif game_mode == GAME_MODE_PLAYER_CHOICE:
            await send_jackpot_game_choice(update, context)
        else:
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("economy", show_economy))
    ...
    application.add_handler(CallbackQueryHandler(handle_jackpot_game_choice, pattern="^jpchoose:"))
    application.add_handler(
        CallbackQueryHandler(
            handle_mines_variation_choice,
            pattern="^(minecfg|minefield|mineset):",
        )
    )
