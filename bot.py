...
def get_game_mode(db: StatsDatabase) -> str:
    ...
    if value in {GAME_MODE_CLASSIC, GAME_MODE_JACKPOT_BUTTONS, GAME_MODE_MINES}:
        return value
...
def get_mines_count(db: StatsDatabase) -> int:
    ...
    try:
        return max(1, min(int(value), button_count - 1))
    except (TypeError, ValueError):
        return min(DEFAULT_MINES_COUNT, button_count - 1)
...
def build_default_mines_prize_steps(db: StatsDatabase) -> list[int]:
    safe_count = get_mines_button_count(db) - get_mines_count(db)
...
def get_mines_prize_steps(db: StatsDatabase) -> list[int]:
    safe_count = get_mines_button_count(db) - get_mines_count(db)
    raw_value = db.get_bot_setting(MINES_PRIZE_STEPS_SETTING, "")
    if raw_value:
        try:
            steps = [int(step) for step in json.loads(str(raw_value))]
        except (TypeError, ValueError, json.JSONDecodeError):
            steps = []
        if len(steps) == safe_count and all(step > 0 for step in steps):
            return steps

    return build_default_mines_prize_steps(db)


def get_mines_prize_steps_text(db: StatsDatabase) -> str:
    return ", ".join(f"{step}⭐" for step in get_mines_prize_steps(db))


def save_mines_prize_steps(db: StatsDatabase, steps: list[int]) -> None:
    safe_count = get_mines_button_count(db) - get_mines_count(db)
    if len(steps) != safe_count:
        raise ValueError(
            f"Нужно указать ровно {safe_count} шагов: кнопок {get_mines_button_count(db)} "
            f"минус мин {get_mines_count(db)}."
        )
    if any(step < 1 for step in steps):
        raise ValueError("Каждый шаг должен быть 1⭐ или выше.")
    if any(steps[index] <= steps[index - 1] for index in range(1, len(steps))):
    ...
    db.set_bot_setting(MINES_PRIZE_STEPS_SETTING, json.dumps(steps))
...
def save_mines_settings(
    ...
    if button_count is not None:
        if button_count < 2:
            raise ValueError("В минках должно быть минимум 2 кнопки.")
    ...
    if mine_count is not None:
        ...
        if mine_count < 1:
            raise ValueError("Мин должна быть минимум 1.")
...
async def send_mines_challenge(
    ...
    button_count = get_mines_button_count(db)
    mine_count = get_mines_count(db)
    prize_steps = get_mines_prize_steps(db)
...
def game_settings_text(db: StatsDatabase) -> str:
    ...
    return (
        ...
        "Лестница минок: /game minessteps 15,20,30,45\n"
    ...
    if context.args:
        ...
        if action in {"classic", "old", "классика", "обычный"} and len(context.args) == 1:
            ...
            if requested_mode in {"classic", "old", "классика", "обычный"}:
                ...
                await update.message.reply_text("Режимы: classic, buttons или mines.")
                return
        elif action in {"mines", "mine", "минки"} and len(context.args) >= 3:
            subaction = context.args[1].lower()
            value_text = " ".join(context.args[2:])
            try:
                value = parse_integer_from_token(value_text, "настройка минок")
...
async def handle_game_settings_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if setting_type == "mode":
        if value not in {GAME_MODE_CLASSIC, GAME_MODE_JACKPOT_BUTTONS, GAME_MODE_MINES}:
            await query.answer()
            return
...
def owner_panel_commands_text() -> str:
    return (
        ...
        "/owner или /panel\n\n"
        "Игра:\n"
        "/game, /game mode classic, /game mode buttons, /game mode mines, /game price 37, "
        "/game buttons 12, /game minbuttons 4, /game decrease 1, "
        "/game nftchance 1/18, /game nftstep 4, /game minesbuttons 9, "
        "/game minescount 2, /game minessteps 15,20,30, /game commission 15, /economy\n\n"
        "Тексты:\n"
